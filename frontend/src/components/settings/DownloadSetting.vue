  ...
  <TabCard :tab="tab">
    ...
    <label class="flex items-center gap-2 input input-bordered">
      ...
      <input
        ...
        min="1"
        max="7"
        v-model.number.lazy="configs.system.download_limit"
        value="5"
        class="range range-xs bg-primary [--range-shdw:#788091]"
        step="1" />
      <span class="pl-2 text-nowrap">{{ configs.system.download_limit }}</span>
    </label>
    <label class="flex items-center gap-2 input input-bordered">
      代理
      <input
        type="text"
        class="ml-2 grow"
        v-model.lazy="configs.system.proxy_url"
        placeholder="请输入代理链接" />

      <input type="checkbox" v-model="configs.system.use_proxy" class="checkbox" />
    </label>
    <label class="flex items-center input input-bordered">
      ...
      <input
        ...
        v-model.lazy="configs.system.download_dir"
        placeholder="设置下载文件夹" />
...
function openDownloadDir() {
  SetDownloadDir('请选择文件夹').then((result) => {
    if (result != '') {
      configs.value.system.download_dir = result
    } else {
